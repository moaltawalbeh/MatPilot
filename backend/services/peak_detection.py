...
This is a real scientific algorithm, not a placeholder.
"""
...
def smooth_savitzky_golay(data: List[float], window_size: int = 5, poly_order: int = 2) -> List[float]:
    ...
    for i in range(len(data)):
        ...
        if sxx == 0:
            smoothed.append(y_mean)
            continue

        sxy = sum(j * window[j] for j in range(n)) - n * x_mean * y_mean
        slope = sxy / sxx
        intercept = y_mean - slope * x_mean

        center = min(max(half_w, i - start), n - 1)
        smoothed.append(intercept + slope * center)
...
def compute_fwhm(intensity: List[float], peak_idx: int, two_theta: List[float]) -> Optional[float]:
    ...
    if left_idx == 0 or right_idx == len(intensity) - 1:
        return None
...
def detect_peaks(
    ...
    smoothed = smooth_savitzky_golay(intensity, window_size=smooth_window)
