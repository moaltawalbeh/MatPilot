...
def generate_report(record: SpectrumRecord, config: Dict[str, Any]) -> Dict[str, Any]:
    cfg = TECHNIQUE_CONFIGS[record.technique]
    results = record.results or {}
    stats = results.get("stats", {})
    lines = [
        f"# {cfg.display_name} — Analysis Report",
        "",
        f"- **Filename:** {record.filename}",
        f"- **Sample ID:** {record.sample_id or 'Unlinked'}",
        f"- **Data points:** {len(record.x)}",
        f"- **Axis:** {cfg.x_axis_label} · {cfg.y_axis_label}",
        f"- **Generated:** {datetime.now(timezone.utc).isoformat()}",
        "",
        "## Processing",
        f"- Smoothing window: {results.get('parameters', {}).get('window', '—')}",
        f"- Baseline order: {results.get('parameters', {}).get('baseline_order', '—')}",
        f"- Peak prominence: {results.get('parameters', {}).get('prominence', '—')}",
        "",
        "## Detected Peaks",
    ]
    peaks = results.get("peaks", [])
    if peaks:
        lines.append("| Position | Intensity | FWHM | Assignment |")
        lines.append("|---|---|---|---|")
        for p in peaks[:20]:
            lines.append(
                f"| {p['position']} | {p['intensity']} | {p.get('fwhm', '—')} | {p.get('assignment') or '—'} |"
            )
    else:
        lines.append("No peaks above the detection threshold were found.")

    lines += [
        "",
        "## Summary Statistics",
        f"- Peak count: {stats.get('peak_count', 0)}",
        f"- SNR: {stats.get('snr', '—')}",
        f"- Max intensity: {stats.get('max_intensity', '—')}",
    ]
    markdown = "\n".join(lines)
    return {
        "spectrum_id": record.id,
        "technique": record.technique,
        "title": f"{cfg.display_name} Report",
        "markdown": markdown,
