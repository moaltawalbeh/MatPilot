    ...
    async def run_phase_identification(
        ...
        for rank, sim in enumerate(similarity_results, 1):
            candidate = {
                ...
                "f_n": round(sim.f_n, 4) if sim.f_n else None,
                "m20": round(sim.m20, 4) if sim.m20 else None,
                "rmse_2theta": round(sim.rmse_2theta, 4) if sim.rmse_2theta else None,
                "mae_2theta": round(sim.mae_2theta, 4) if sim.mae_2theta else None,
                ...
                "n_unexplained_exp": sim.n_unexplained_exp,
                "n_missing_ref": sim.n_missing_ref,
                "global_shift": round(sim.global_shift, 4),
                "quality_mark": sim.quality_mark or None,
                "quality_prior": round(sim.quality_prior, 4) if sim.quality_prior else None,
                "raw_match_score": round(sim.raw_match_score, 4) if sim.raw_match_score else None,
